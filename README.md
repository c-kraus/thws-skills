...
| **marp-slides** | `/marp-slides` | Erstellt MARP-Präsentationsfolien im THWS-Corporate-Design aus Gliederungen, Rohmaterial oder fertigen `.qmd`-Kapiteln. Statisch, schnell, ohne Setup — die kürzere Route, wenn keine Interaktion gebraucht wird. | — |
| **revealjs-slides** | `/revealjs-slides` | Erstellt interaktive Quarto-Reveal.js-Folien im THWS-Design (Format-Extension `c-kraus/thws-revealjs`): Zwischenfragen mit Quiz-Plugin, Tabsets, Hervorhebungen, Laserpointer, Speaker Notes und PDF-Handout aus derselben Datei. Für Sitzungen mit Interaktion oder Web-Auslieferung. | `quarto` ≥ 1.9.36 |
...
| Skill | Aufruf | Was er tut | Benötigt |
|---|---|---|---|
| **tutorial-builder** | `/tutorial-builder` | Erstellt knappe Übungsblätter und Einzelaufgaben als `.qmd` im **`tutorial-typst`**-Format (eine Datei, Single Source: Angabe- + Lösungsblatt über `show_solutions`-Toggle). Struktur: wenige `## Aufgabe N:`-Blöcke mit `#### Task N:`-Teilaufgaben. Fünf Aufgabentypen (Rechnen, MC, Kurzszenario, Verständnis, Buchung/Norm), Schwierigkeitsprogression, Python-Arithmetik-Check, Lernziel-Abgleich. Aufgaben auf Wunsch an `exam-builder` exportierbar. | `tutorial-typst`-Extension im Projekt (`_extensions/tutorial/`) |
