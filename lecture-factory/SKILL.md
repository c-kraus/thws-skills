...
1. **Curriculum:** `_curriculum.md` im Ausgabe-/Projektverzeichnis suchen. Gefunden → still laden: Zielgruppen-Block, Vorgänger-/Nachfolger-Kapitel (Titel + Kernthemen), Terminologie-Konventionen sowie — falls vorhanden — die optionale Metazeile `**Wiki:** <pfad>`. Ist sie gesetzt und der Pfad existiert → **Wiki-Modus** aktiv (siehe Schritt 3); fehlt sie → klassischer Ablauf, alle Wiki-Schritte entfallen still. Nicht gefunden → **einmalig aktiv fragen**: „Ich habe keine `_curriculum.md` gefunden — sie definiert Zielgruppe, Kapitelfolge und Terminologie. Soll ich sie mit dem `curriculum-architect` Skill anlegen?" Bei Nein: mit Default-Zielgruppe fortfahren (siehe quarto-lecture).
