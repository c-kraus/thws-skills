...
1. **Curriculum:** `_curriculum.md` im Ausgabe-/Projektverzeichnis suchen. Gefunden → still laden: Zielgruppen-Block, Vorgänger-/Nachfolger-Kapitel (Titel + Kernthemen), Terminologie-Konventionen sowie die Metazeile `**Wiki:** <pfad>`. **Wiki-Erkennung (fragen & merken):** Ist die Zeile gesetzt und der Pfad existiert → **Wiki-Modus** aktiv (siehe Schritt 3). Ist `**Wiki:** none` gesetzt → klassischer Ablauf, nicht erneut fragen. **Fehlt die Zeile ganz → einmal fragen** „Wo liegt das Wiki für diesen Kurs? (absoluter Pfad — oder ‚keins')" und die Antwort **selbst** ins `_curriculum.md` schreiben (`**Wiki:** <pfad>` bzw. `**Wiki:** none`), direkt unter `**Sprache:**`. So merkt sich der Skill die Wahl; der Nutzer editiert nie von Hand. Bei genanntem Pfad → Wiki-Modus; bei „keins"/`none` → klassischer Ablauf, alle Wiki-Schritte entfallen still. Nicht gefunden (kein `_curriculum.md`) → **einmalig aktiv fragen**: „Ich habe keine `_curriculum.md` gefunden — sie definiert Zielgruppe, Kapitelfolge und Terminologie. Soll ich sie mit dem `curriculum-architect` Skill anlegen?" Bei Nein: mit Default-Zielgruppe fortfahren (siehe quarto-lecture).
